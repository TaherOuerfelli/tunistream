...
//  const [theme , setTheme] = useState('dark');
  ...
  const handleTimeUpdate = () => {
    ...
    if (videoElement && videoElement.readyState > 1 && !videoElement.paused) {
      ...
      if (mediaType === 'movie') {
        if (!mediaData['m' + mediaID]) {
          mediaData['m' + mediaID] = {time:0,progress:0};
        }
        const moviedata = mediaData['m' + mediaID] as MovieData;
        moviedata.time= Math.floor(videoElement.currentTime)
        moviedata.progress= Math.floor(progress * 0.01) === 0 && progress !== 0 ? 1 : Math.floor(progress * 0.01)
        mediaData={['m' + mediaID]:mediaData['m' + mediaID],...mediaData};
      } else {
        const seasonKey = 's' + mediaID;
